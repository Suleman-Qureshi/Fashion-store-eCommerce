...
function Page() {
  ...
  <SelectContent>
    {statustype.map((item,index)=><SelectItem key={index} value={item.type}>{item.type}</SelectItem>)}
  </SelectContent>
</Select>
              <Select>
  <SelectTrigger className="w-[180px]">
  ...
  <SelectContent>
    {duration.map((item,index)=><SelectItem key={index} value={item.type}>{item.type}</SelectItem>)}
  </SelectContent>
</Select>
        </div>
      </div>
    {userId ? (
        ...
        <p className="text-gray-500 mt-10 text-center">Loading your orders...</p>
      )}
    </section>
  )
}
