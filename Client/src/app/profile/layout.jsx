  ...
  const fetchUsername=async ()=>{
    ...
    try{
      ...
      if(response.data.avatar){
        setAvatarUrl(`http://localhost:5000/uploads/${response.data.avatar}`);
      }else{
        setAvatarUrl(null)
      }
    }catch(error){
...
const deleteAvatar=async ()=>{
  try{
     await axios.put(`http://localhost:5000/api/user/${userId}/remove-avatar`);
     setAvatarUrl(null);
  }catch(error){
  ...
  return (
    ...
    <div className="w-screen flex flex-col justify-center items-center tertiary-color">
     <div className="flex flex-col gap-2 w-[67rem] py-8">
    ...
    <div className="flex w-[67rem]">
      <aside className="w-64 py-4 flex flex-col gap-4">
