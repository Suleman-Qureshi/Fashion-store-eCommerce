...
function Page() {
  ...
  const fetchingAddressData = async () => {
    if (!userId) return;
    try {
      const response = await axios.get(`https://your-api.com/api/address/${userId}`);
  ...
  const deleteAddress = async (addressId) => {
    try {
      await axios.delete(`https://your-api.com/api/address/${addressId}/delete`);
      alert("🗑️ Address deleted successfully");
      setAddressData((prev) => prev.filter((address) => address._id !== addressId));

      await axios.post("https://your-api.com/api/notification", {
        userId,
  ...
  const makeDefaultAddress = async (addressId) => {
    try {
      await axios.put(`https://your-api.com/api/address/${addressId}`);
