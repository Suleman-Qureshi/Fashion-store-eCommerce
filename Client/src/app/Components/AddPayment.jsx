...
function AddPayment({setFormActive,refreshCard}) {
    ...
    const formatCardNumber=value=>{var numberOnly= value.replace(/\D/g, "");return numberOnly = numberOnly.replace(/(\d{4})(?=\d)/g, "$1 ");}
      const onSubmit = async (data) => {
        ...
        try{axios.post("http://localhost:5000/api/cards",{userId,last4digit,expiryMonth:paddedMonth,expiryYear,brand,});alert("Payment Method added");await refreshCard();reset();setFormActive(false)}catch(error){console.log(error)};}
  return (
    <>
